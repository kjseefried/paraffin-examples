--
--  Copyright (C) 2012,2013 Michael Erdmann
--
...
package body Time_Measurement is
   ...
   --  This information is stored in each task. The information is merged
   --  into the global summary after the task has terminated.
   ...
   subtype Clock_Index_Type is
     Clock_Id_Type range (Clock_Id_Null + 1) .. Clock_Id_Type'Last;

   Null_Timer : constant Timer_Record_Type :=
     (Samples => 0, Start_Time => Time_First, Elapsed => 0.0);

   type Clock_Array_Type  is array (Clock_Id_Type) of Timer_Record_Type;
   type Clock_Array_Access is access all Clock_Array_Type;

   package Per_Task is new Ada.Task_Attributes
     (Attribute => Clock_Array_Access, Initial_value => null);
   use Per_Task;
   ...
   protected type Summary_Type is
      function To_String
        (Id : Clock_Id_Type) return String;
      procedure Finish
        (C : Cause_Of_Termination; T : Task_Id; X : Exception_Occurrence);
      procedure Update
        (Id : Clock_Id_Type; Samples : Natural; Elapsed : Duration);
      procedure Register
        (Name : String; Result : out Clock_Id_Type);
      procedure List_Registered;
   ...
   protected body Summary_Type is
      ...
      procedure Update (Id : Clock_Id_Type;
                        Samples : Natural;
                        Elapsed : Duration) is
      begin
      ...
      procedure Register (Name : String; Result : out Clock_Id_Type) is
      begin
         for Id in Clock_Index_Type loop
            if Clock (Id).Name = Null_Unbounded_String then
      ...
      begin
         for I in Clock_Index_Type'Range loop
            if Clock (I).Name /= Null_Unbounded_String then
               Log.Comment (To_String (I));
            end if;
         end loop;
      end List_Registered;

      function To_String (Id : Clock_Id_Type) return String is
      ...
      begin
         if Clock (Id).Samples > 0 then
            return
              Format (To_String (Clock (Id).Name))
            & Calendar.Formatting.Image (Elapsed_Time => Clock (Id).Elapsed,
                                           Include_Time_Fraction => True)
            & Duration'Image
              (Clock (Id).Elapsed / Duration (Clock (Id).Samples))
            & Natural'Image (Clock (Id).Samples);
         else
            return
              Format (To_String (Clock (Id).Name))
              & " ** not executed ** ";
      ...
      -- Finish --
      ------------
      procedure Finish (C : Cause_Of_Termination;
                        T : Task_Id;
      ...
      begin
         for Id in Clock_Id_Type loop
            if My_Clock (Id).Samples > 0 then
               Clock (Id).Samples
                    := Clock (Id).Samples + My_Clock (Id).Samples;
               Clock (Id).Elapsed
                    := Clock (Id).Elapsed + My_Clock (Id).Elapsed;
            end if;
         end loop;
      end Finish;
   ...
   -- Register --
   --------------
   function Register (Name : String) return Clock_Id_Type is
   ...
   -- Start_lap --
   ----------------
   procedure Start_Lap (Id : Clock_Id_Type) is
   ...
   -- Cancel_Lap --
   ----------------
   procedure Cancel_Lap (Id : Clock_Id_Type) is
      Clock   : constant Clock_Array_Access := Per_Task_Data;
      Current : constant Real_Time.Time := Real_Time.Clock;
   begin
      null;
